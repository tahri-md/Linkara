...
const resolveMaxRetry = async (payload: JobExecutionPayload): Promise<number> => {
	...
	return typeof workflowJob?.retry_count === 'number' ? workflowJob.retry_count : 0;
};

const scheduleRetry = async (queue: Queue, queueName: string, jobId: string, failedReason?: string): Promise<void> => {
	...
	const retryCount = jobStateResult.rows.length > 0 && typeof jobStateResult.rows[0].retry_count === 'number'
	...
	await queue.add(queuedJob.name, {
		...payload,
		retryCount,
		maxRetry,
	}, {
...
export async function initializeQueueInfrastructure(): Promise<void> {
	...
	jobExecutionQueueEvents.on('failed', ({ jobId, failedReason }) => {
		...
		void scheduleRetry(jobExecutionQueue, queueNames.jobExecution, jobId, failedReason).catch((err) => {
			console.error(`[queue:${queueNames.jobExecution}] Retry handler error:`, err);
		});
	});
