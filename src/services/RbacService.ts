...
export class RBACService {
    ...
    try {
      ...
      for (const roleRow of result.rows) {
        ...
        roles.push({
          ...
          permissions: permResult.rows.map(
  ...
  async listRoles(orgId: string): Promise<Role[]> {
    try {
      const result = await query(
        "SELECT id, org_id, name, description, is_predefined, created_at, updated_at FROM roles WHERE org_id = $1 ORDER BY name",
        [orgId],
      );
      ...
      throw new Error(
        `Failed to list roles: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async updateRolePermissions(
    ...
    try {
      ...
      await query("UPDATE roles SET updated_at = NOW() WHERE id = $1", [
        roleId,
      ]);
      ...
      throw new Error(
        `Failed to update role permissions: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async deleteRole(roleId: string): Promise<void> {
    try {
      const roleResult = await query(
        "SELECT is_predefined FROM roles WHERE id = $1",
        [roleId],
      );
  ...
  async removeRoleFromMember(
    ...
    try {
      await query(
        "DELETE FROM member_roles WHERE org_id = $1 AND user_id = $2 AND role_id = $3",
        [orgId, userId, roleId],
      );
    } catch (error) {
      throw new Error(
