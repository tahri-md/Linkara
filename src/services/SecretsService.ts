...
export class SecretsService {
    ...
    private formatSecret(row: SecretRecord): SecretRecord {
        return {
            ...
            accessed_at: row.accessed_at ? (row.accessed_at instanceof Date ? row.accessed_at : new Date(row.accessed_at)) : null,
        };
    }
    ...
    async getSecret(orgId: string, secretId: string, userId: string): Promise<string> {
        await this.assertOrgOwner(orgId, userId);
        ...
        const plaintext = decryptSecret(secret.encrypted_value, this.getEncryptionKey());
        ...
        await this.logAudit(orgId, userId, 'secret.accessed', secretId, { name: secret.name });
        return plaintext;
    }
    ...
    async getSecretForJob(jobId: string, secretName: string): Promise<string> {
        ...
        await this.logAudit(secret.org_id, null, 'secret.accessed_by_job', secret.id, {
            jobId,
            name: secret.name,
        });
    ...
    async rotateSecret(
        ...
        const secret = this.formatSecretRow(result.rows[0]);
        await this.logAudit(orgId, userId, 'secret.updated', secret.id, { name: secret.name });
        return secret;
    }
    ...
    async deleteSecret(orgId: string, secretId: string, userId: string): Promise<void> {
        await this.assertOrgOwner(orgId, userId);
